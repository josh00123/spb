...
def job_details(job_id):
    ...
    connection.execute("SELECT s.service_name, js.qty FROM job_service js JOIN service s ON js.service_id = s.service_id WHERE js.job_id = %s;", (job_id,))
    services_used = connection.fetchall()

    connection.execute("SELECT p.part_name, jp.qty FROM job_part jp JOIN part p ON jp.part_id = p.part_id WHERE jp.job_id = %s;", (job_id,))
